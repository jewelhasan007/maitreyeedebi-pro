...
// Local thumbnail images served ONLY from the /public folder.
// Drop files into public/performances/perf-1.jpg, perf-2.jpg, etc.
// and reference them here with a root-relative path (no "public/" prefix).
// Matched to PERFORMANCES by index — item 0 gets PERFORMANCE_THUMBNAILS[0], etc.
// No fallback to perf.thumbnailUrl: if there's no entry for an index, that
// performance is skipped from the grid entirely.
const PERFORMANCE_THUMBNAILS: string[] = [
  '/performances/imge001.png',
  '/performances/perf-2.jpg',
  '/performances/perf-3.jpg',
  '/performances/perf-4.jpg',
];

const getThumbnail = (index: number): string | undefined =>
  PERFORMANCE_THUMBNAILS[index];
    ...
    <section id="performances" className="py-16 sm:py-20 relative bg-[#090909]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map((perf) => {
            const originalIndex = PERFORMANCES.findIndex((p) => p.id === perf.id);
            const thumbnail = getThumbnail(originalIndex);

            // No local image for this performance — skip rendering its card.
